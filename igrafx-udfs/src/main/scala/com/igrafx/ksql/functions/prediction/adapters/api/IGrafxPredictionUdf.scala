...
import com.igrafx.ksql.functions.prediction.adapters.api.dtos._
import com.igrafx.ksql.functions.prediction.adapters.services.IGrafxApiServiceImpl
import com.igrafx.ksql.functions.prediction.domain.entities.exceptions.{
  InvalidTokenException,
  NoMoreTryException,
  PredictionException
}
import com.igrafx.ksql.functions.prediction.domain.usecases.PredictionUseCases
...
@UdfDescription(
  name = "igrafx_prediction",
  author = "iGrafx",
  description = "Udf allowing to retrieve prediction information on given caseIds belonging to a iGrafx project"
)
class IGrafxPredictionUdf {
  private val log: Logger = LoggerFactory.getLogger(getClass)

  private val predictionUseCases: PredictionUseCases = new PredictionUseCases(new IGrafxApiServiceImpl)

  @Udf(
    description = "Retrieves prediction information for caseIds related to a iGrafx Project",
    schema = PredictionStructs.STRUCT_OUTPUT_SCHEMA_DESCRIPTOR
  )
  def igrafxPrediction(
      @UdfParameter(
        value = "caseIds",
      ...
      @UdfParameter(
        value = "workgroupId",
        description = "The id of the iGrafx workgroup related to the project"
      ) workgroupId: String,
      @UdfParameter(
        value = "workgroupKey",
        description = "The key of the iGrafx workgroup related to the project"
      ) workgroupKey: String,
      @UdfParameter(
        value = "apiUrl",
        description = "The URL corresponding to the iGrafx API (for instance : https://mining-api.dev2.igrafxcloud.com)"
      ) apiUrl: String,
      @UdfParameter(
        value = "authUrl",
        description =
          "The URL corresponding to the iGrafx Authentication API (for instance : https://id.dev2.igrafxcloud.com/realms/logpickr-api-mining/)"
      ) authUrl: String,
      @UdfParameter(
        value = "tryNumber",
        description =
    ...
    Try {
        ...
        predictionUseCases
          ...
          .recover {
            ...
            case exception: Throwable =>
              log.error("Unexpected Exception while trying to retrieve Predictions from the iGrafx API", exception)
              throw exception
          }
      val predictions =
      ...
      case Failure(exception: concurrent.TimeoutException) =>
        log.error("Timeout exceeded for the iGrafx prediction", exception)
        throw exception
      ...
      case Failure(exception: DataException) =>
        ...
        throw exception
      case Failure(exception: Throwable) =>
        log.error("Unexpected issue with the iGrafx Prediction UDF", exception)
        throw exception
    }
  }
