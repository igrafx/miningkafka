package com.igrafx.ksql.functions.prediction.domain.usecases
...
class PredictionUseCases(private val igrafxApiService: IGrafxApiService) {
  ...
  def getPrediction(
    ...
    val predictionDescription =
      s"prediction on the $projectId iGrafx Project for the following Case Ids : $caseIds"
    (for {
      token <- igrafxApiService.sendAuthentication(authUrl, workgroupId, workgroupKey, predictionDescription)
      predictionUrl <- igrafxApiService.sendPrediction(apiUrl, caseIds, projectId, token, predictionDescription)
  ...
  private def pullPredictionResult(
    ...
    (for {
      token <- igrafxApiService.sendAuthentication(authUrl, workgroupId, workgroupKey, predictionDescription)
      predictionsOpt <- igrafxApiService.getPredictionResult(predictionUrl, token, predictionDescription)
