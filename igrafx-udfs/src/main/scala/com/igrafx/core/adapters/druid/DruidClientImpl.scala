...
import com.igrafx.ksql.functions.caseevents.Constants.{sqlJsonParameterTypeName, sqlJsonParameterValueName}
  ...
  private def sendJsonQuery(
    ...
    Future {
      ...
      if (druidResponse.isSuccess) {
        ...
        log.error(
          s"[UDF igrafx_case_events] Error in the Http Response : $errorDescription for the query : $jsonQuery"
            .replaceAll("[\r\n]", "")
        )
      ...
      case exception =>
        log.error(
          s"[UDF igrafx_case_events] Problem with the Http Request for the query $jsonQuery".replaceAll("[\r\n]", ""),
          exception
        )
        throw exception
    }
