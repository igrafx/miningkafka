...
import com.igrafx.ksql.functions.sessions.Constants
import com.igrafx.ksql.functions.sessions.domain.structs.Structs
...
final class IGrafxSessionsUdtfTest extends UnitTestSpec {
  val igrafxSessionsUdtf = new IGrafxSessionsUdtf()
  ...
  describe("igrafxSessions") {
    ...
    it("should return a Java List with the good results if isIgnoreIfNoStart is true and isIgnoreIfNoEnd is true") {
      ...
      val res = igrafxSessionsUdtf.igrafxSessions(
        listTest,
        ".*;.*;.*;ignoreLine",
        ".*;(.*);.*;.*",
        ".*;.*;.*;Start",
  ...
  describe("getFilteredLines") {
    it("should filter the lines and remove those matching ignorePattern") {
      ...
      assert(expectedResultSeq == igrafxSessionsUdtf.getFilteredLines(inputSeq, ".*;.*;.*;ignoreLine"))
    }
  }
  ...
  describe("getSessionLineStruct") {
    it(s"should create a Struct ${Structs.STRUCT_OUTPUT_SCHEMA_DESCRIPTOR} from a sessionId and a line") {
      ...
      assert(expectedResult == igrafxSessionsUdtf.getSessionLineStruct("sessionIdTest", "sessionLineTest"))
    }
  }
  ...
  describe("getGroup") {
    it(
      ...
      val res = igrafxSessionsUdtf.getGroup(
    ...
    it("should return None in case of incoherence between the groupSessionPattern and the session line") {
      ...
      val res2 = igrafxSessionsUdtf.getGroup(
        "2020-06-16T04;1;appTest1;Start",
        ".*;.*;.*;.*".r
      )
      res2 match {
      ...
      val res3 = igrafxSessionsUdtf.getGroup(
        "2020-06-16T04;1;appTest1;Start",
        "".r
      )
      res3 match {
      ...
      val res4 = igrafxSessionsUdtf.getGroup(
  ...
  describe("getSessionId") {
    it(
      ...
      val res = igrafxSessionsUdtf.getSessionId(
        "2020-06-16T04;1;appTest1;Start",
        ".*;(.*);(.*);.*".r,
        isSessionIdHash = false
      )
    ...
    it(
      ...
      val res2 =
        igrafxSessionsUdtf.getSessionId("2020-06-16T04;1;appTest1;Start", ".*;.*;(.*);.*".r, isSessionIdHash = true)
    ...
    it(
      ...
      assert(
        "" == igrafxSessionsUdtf.getSessionId(
          "2020-06-16T04;1;appTest1;Start",
          ".*;.*;.*;.*;(.*)".r,
          isSessionIdHash = true
        )
