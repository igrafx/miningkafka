...
import com.igrafx.kafka.sink.aggregationmain.domain.entities.{DebugInformation, Event, PartitionTracker}
...
class PartitionTrackerMock extends Mock[PartitionTracker] {
  ...
  private var pendingValue: Seq[Event] = Seq(new EventMock().build())
  ...
  def setPendingValue(pendingValue: Seq[Event]): PartitionTrackerMock = {
